...
const DEVICE_COOKIE = "GL_DEVICE_ID";
...
async function createAdminSession(secret) {
  const payload = { exp: Math.floor(Date.now() / 1000) + ADMIN_SESSION_MAX_AGE, nonce: crypto.randomUUID() };
...
async function createUserSession(env, accessToken, deviceId) {
  ...
  await env.GuitarLabAccess.put(`session:${sessionId}`, JSON.stringify(sessionRecord), { expirationTtl: USER_SESSION_MAX_AGE });
  return sessionId;
}
...
async function registerDevice(env, accessToken, deviceId) {
  ...
  const device = { id: deviceId, createdAt: now, lastAccessAt: now, revoked: false };
  await env.GuitarLabAccess.put(`device:${accessToken}:${deviceId}`, JSON.stringify(device));
  return device;
}
...
async function recordDeviceAttempt(env, accessToken, deviceId) {
  ...
  await env.GuitarLabAccess.put(`device-attempt:${accessToken}:${attemptId}`, JSON.stringify(record));
}
...
async function getValidUserSession(request, env) {
  ...
  if (!session || !session.accessToken || session.deviceId !== deviceId) return null;
  const access = await env.GuitarLabAccess.get(`access:${session.accessToken}`, "json");
  if (!access || access.revoked === true) return null;
  if (access.expiresAt && new Date(access.expiresAt).getTime() <= Date.now()) return null;
  const device = await env.GuitarLabAccess.get(`device:${session.accessToken}:${deviceId}`, "json");
  if (!device || device.revoked === true) return null;
  return { sessionId, access, device };
}
...
async function authorizeRecoveredDevice(env, access, token, request) {
  ...
  for (const device of devices) await env.GuitarLabAccess.delete(`device:${token}:${device.id}`);
  ...
  access.accessCount = Number.isFinite(access.accessCount) ? access.accessCount + 1 : 1;
  ...
  responseHeaders.append("Set-Cookie", `${USER_SESSION_COOKIE}=${sessionId}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=${USER_SESSION_MAX_AGE}`);
  responseHeaders.append("Set-Cookie", `${DEVICE_COOKIE}=${deviceId}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=${DEVICE_COOKIE_MAX_AGE}`);
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/admin" && request.method === "GET") {
      if (await isValidAdminSession(request, env.ADMIN_PASSWORD)) return adminDashboard(env);
    ...
    if (url.pathname === "/admin/login" && request.method === "POST") {
      ...
      if (typeof password !== "string" || password !== env.ADMIN_PASSWORD) return adminLoginPage("Password non corretta.");
      const session = await createAdminSession(env.ADMIN_PASSWORD);
      return new Response(null, { status: 303, headers: { "Location": "/admin", "Set-Cookie": `${ADMIN_SESSION_COOKIE}=${session}; HttpOnly; Secure; SameSite=Strict; Path=/admin; Max-Age=${ADMIN_SESSION_MAX_AGE}`, "Cache-Control": "no-store" } });
    }

    if (url.pathname === "/admin/access/create" && request.method === "POST") {
      if (!(await isValidAdminSession(request, env.ADMIN_PASSWORD))) return adminLoginPage("Sessione amministrativa non valida o scaduta.");
      const formData = await request.formData();
      const label = formData.get("label");
      const expiresAtInput = formData.get("expiresAt");
      if (typeof label !== "string" || !label.trim()) return adminDashboard(env, "Inserisci un nome o un'etichetta.");
      let expiresAt = null;
      if (typeof expiresAtInput === "string" && expiresAtInput.trim()) {
        const parsed = new Date(expiresAtInput);
        if (Number.isNaN(parsed.getTime())) return adminDashboard(env, "La data di scadenza non è valida.");
        if (parsed.getTime() <= Date.now()) return adminDashboard(env, "La scadenza deve essere nel futuro.");
        expiresAt = parsed.toISOString();
      }
      const token = crypto.randomUUID().replaceAll("-", "");
      const recoveryPin = generateRecoveryPin();
      const record = { token, label: label.trim(), createdAt: new Date().toISOString(), expiresAt, revoked: false, lastAccessAt: null, accessCount: 0, maxDevices: DEFAULT_MAX_DEVICES, recoveryPin };
      await env.GuitarLabAccess.put(`access:${token}`, JSON.stringify(record));
      const accessUrl = new URL(`/access/${token}`, url.origin).toString();
      return adminDashboard(env, "Accesso creato correttamente.", accessUrl, recoveryPin);
    }
    ...
    if (url.pathname === "/admin/access/reactivate" && request.method === "POST") {
      ...
      if (typeof token !== "string" || !token || token.includes("/")) return adminDashboard(env, "Accesso da riattivare non valido.");
    ...
    if (url.pathname === "/admin/access/delete" && request.method === "POST") {
      ...
      const formData = await request.formData();
      const token = formData.get("token");
      if (typeof token !== "string" || !token || token.includes("/")) return adminDashboard(env, "Accesso da eliminare non valido.");
      const accessKey = `access:${token}`;
      const access = await env.GuitarLabAccess.get(accessKey, "json");
      if (!access) return adminDashboard(env, "Accesso non trovato.");
      const isExpired = access.expiresAt && new Date(access.expiresAt).getTime() <= Date.now();
      if (access.revoked !== true && !isExpired) return adminDashboard(env, "Puoi eliminare solo accessi revocati o scaduti.");
      await env.GuitarLabAccess.delete(accessKey);
      const deviceKeys = await env.GuitarLabAccess.list({ prefix: `device:${token}:` });
      for (const key of deviceKeys.keys) await env.GuitarLabAccess.delete(key.name);
      const attemptKeys = await env.GuitarLabAccess.list({ prefix: `device-attempt:${token}:` });
      for (const key of attemptKeys.keys) await env.GuitarLabAccess.delete(key.name);
    ...
    if (url.pathname.startsWith("/access/") && url.pathname.endsWith("/recover") && request.method === "POST") {
      const parts = url.pathname.split("/").filter(Boolean);
      const token = parts.length === 3 && parts[0] === "access" && parts[2] === "recover" ? parts[1] : "";
      if (!token) return accessDeniedPage("Link di recupero non valido.");
      const access = await env.GuitarLabAccess.get(`access:${token}`, "json");
      if (!access || access.revoked === true) return accessDeniedPage("Questo accesso non è più disponibile.");
      if (access.expiresAt && new Date(access.expiresAt).getTime() <= Date.now()) return accessDeniedPage("Questo accesso è scaduto.");
      if (!access.recoveryPin) return accessDeniedPage("Il recupero non è disponibile per questo accesso.");
      const formData = await request.formData();
      const pin = formData.get("pin");
      if (typeof pin !== "string" || !/^\d{6}$/.test(pin) || pin !== access.recoveryPin) {
        const deviceId = getCookie(request, DEVICE_COOKIE) || crypto.randomUUID().replaceAll("-", "");
        await recordDeviceAttempt(env, token, deviceId);
        return recoveryPage(token, "PIN di recupero non corretto.");
      }
      return authorizeRecoveredDevice(env, access, token, request);
    }

    if (url.pathname === "/access" || url.pathname.startsWith("/access/")) {
      const token = url.pathname.slice("/access/".length);
      if (url.pathname.endsWith("/recover")) return accessDeniedPage("Link di recupero non valido.");
      if (request.method !== "GET" || !token || token.includes("/")) return accessDeniedPage("Link di accesso non valido.");
      const access = await env.GuitarLabAccess.get(`access:${token}`, "json");
      if (!access || access.revoked === true) return accessDeniedPage("Questo accesso non è più disponibile.");
      if (access.expiresAt && new Date(access.expiresAt).getTime() <= Date.now()) return accessDeniedPage("Questo accesso è scaduto.");
      const deviceId = getCookie(request, DEVICE_COOKIE) || crypto.randomUUID().replaceAll("-", "");
      ...
      if (existingDevice && existingDevice.revoked !== true) {
        ...
        const devices = await getRegisteredDevices(env, token);
        const maxDevices = Number.isInteger(access.maxDevices) && access.maxDevices > 0 ? access.maxDevices : DEFAULT_MAX_DEVICES;
        if (devices.length === 0) {
          await registerDevice(env, token, deviceId);
        } else {
          await recordDeviceAttempt(env, token, deviceId);
          return recoveryPage(token);
        }
      } else {
        await recordDeviceAttempt(env, token, deviceId);
    ...
    if (userSession) return env.ASSETS.fetch(request);
